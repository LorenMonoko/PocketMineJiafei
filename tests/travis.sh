...
rm -rf "$DATA_DIR"
rm PocketMineJiafei.phar 2> /dev/null
...
"$PHP_BINARY" -dphar.readonly=0 DevTools.phar --make src,vendor --relative ./ --entry src/pocketmine/PocketMine.php --out PocketMineJiafei.phar
if [ -f PocketMineJiafei.phar ]; then
...
echo -e "stop\n" | "$PHP_BINARY" PocketMineJiafei.phar --no-wizard --disable-ansi --disable-readline --debug.level=2 --data="$DATA_DIR" --plugins="$PLUGINS_DIR" --anonymous-statistics.enabled=0 --settings.async-workers="$PM_WORKERS" --settings.enable-dev-builds=1
