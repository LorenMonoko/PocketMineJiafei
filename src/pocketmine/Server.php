...
/**
 * PocketMineJiafei is the Minecraft: PE multiplayer server software
...
class Server{
	...
	public function __construct(\ClassLoader $autoloader, \AttachableThreadedLogger $logger, string $dataPath, string $pluginPath){
		...
		try{
			...
			if(\pocketmine\IS_DEVELOPMENT_BUILD and !((bool) $this->getProperty("settings.enable-dev-builds", false))){
				...
				$this->logger->emergency($this->baseLang->translateString("pocketmine.server.devBuild.error5", ["https://github.com/pmmp/PocketMineJiafei/releases"]));
				$this->forceShutdown();
				return;
			}
	...
	/**
	 * Starts the PocketMineJiafei server and starts processing ticks and packets
	 */
