...
This release is the first to use the new merged versioning system. The PocketMineJiafei version and the API version are now one and the same.
...
## Core
### General
- This release introduces a new dependency library called `Snooze`, which PocketMineJiafei utilizes for managing notifications between threads. This library is currently utilized by command reading, RCON and RakLib, allowing faster response times to commands, reduced network latency, and better performance due to lack of need for polling.
...
The way that the API versioning is done has changed. Now the API version is synonymous with the PocketMineJiafei version.
